    ...
    if (usePlugins.length === 1 && usePlugins[0] === constants_1.USE_ALL_PLUGINS) {
        ...
        for (const pluginName of usePlugins) {
            if (pluginName in pluginConfig.installedExtensions) {
                ...
                throw new Error(`Could not load the plugin '${pluginName}' because it is not installed. ` +
                    `Only the following plugins are available: ${lodash_1.default.keys(pluginConfig.installedExtensions)}`);
            }
        }
    }
    ...
    if (lodash_1.default.isEmpty(useDrivers)) {
        ...
        for (const driverName of useDrivers) {
            if (driverName in driverConfig.installedExtensions) {
                ...
                throw new Error(`Could not load the driver '${driverName}' because it is not installed. ` +
                    `Only the following drivers are available: ${lodash_1.default.keys(driverConfig.installedExtensions)}`);
            }
        }
    }
